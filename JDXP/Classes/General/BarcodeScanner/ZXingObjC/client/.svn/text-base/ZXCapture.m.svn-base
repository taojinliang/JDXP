...
- (void)dealloc {
    ...
    for (AVCaptureConnection* conn in _output.connections) {
              ...
              dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                  ...
                  if (strongSelf) {
                      @synchronized(strongSelf) {
                          if (![strongSelf.session.outputs containsObject:weakSelf.output]) {
                              if ([strongSelf.session canAddOutput:weakSelf.output]) {
                                  [strongSelf.session addOutput:weakSelf.output];
                              }
                          }
                      }
                  }
