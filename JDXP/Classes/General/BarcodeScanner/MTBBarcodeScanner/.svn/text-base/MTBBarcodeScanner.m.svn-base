...
- (void)startScanningWithResultBlock:(void (^)(NSArray *codes))resultBlock {
    ...
    for (AVCaptureOutput* output in self.session.outputs) {
        if ([output isKindOfClass:[AVCaptureMetadataOutput class]]) {
            AVCaptureMetadataOutput* metadataOutput = (id)output;
            
            static const CGRect kFullRectOfInterest = (CGRect){0, 0, 1, 1};
            CGRect rectOfInterest = kFullRectOfInterest;
            if (self.scanRect) {
//                rectOfInterest = [self.capturePreviewLayer metadataOutputRectOfInterestForRect:self.scanRect.CGRectValue];
                
                NSParameterAssert([self.session.sessionPreset isEqualToString:AVCaptureSessionPreset640x480]);
                CGSize kSessionSize = CGSizeMake(640, 480);
                { // swap
                    CGFloat temp = kSessionSize.width;
                    kSessionSize.width = kSessionSize.height;
                    kSessionSize.height = temp;
                }
                
                CGSize size = [UIScreen mainScreen].bounds.size;
                CGRect cropRect = self.scanRect.CGRectValue;
                CGFloat p1 = size.height / size.width;
                CGFloat p2 = kSessionSize.height / kSessionSize.width;
                if (p1 < p2) {
                    CGFloat fixHeight = size.width * kSessionSize.height / kSessionSize.width;
                    CGFloat fixPadding = (fixHeight - size.height) / 2;
                    rectOfInterest =
                    CGRectMake((cropRect.origin.y + fixPadding) / fixHeight,
                               cropRect.origin.x / size.width,
                               cropRect.size.height / fixHeight,
                               cropRect.size.width/size.width);
                } else {
                    CGFloat fixWidth = size.height * kSessionSize.width / kSessionSize.height;
                    CGFloat fixPadding = (fixWidth - size.width) / 2;
                    rectOfInterest = CGRectMake(cropRect.origin.y/size.height,
                                                              (cropRect.origin.x + fixPadding)/fixWidth,                                            cropRect.size.height/size.height,                                              cropRect.size.width/fixWidth);
                }
                
//                
//                CGSize screenSize = [UIScreen mainScreen].bounds.size;
//                CGRect scanRectValue = self.scanRect.CGRectValue;
//                CGRect cropRect = kFullRectOfInterest;
//                cropRect.origin.x = scanRectValue.origin.y / screenSize.height;
//                cropRect.origin.y = scanRectValue.origin.x / screenSize.width;
//                cropRect.size.width = scanRectValue.size.height / screenSize.height;
//                cropRect.size.height = scanRectValue.size.width / screenSize.width;
//                metadataOutput.rectOfInterest = cropRect;
//                
//                break; // <=====
//                
//                if (CGRectIsEmpty(rectOfInterest)) {
//                    rectOfInterest = kFullRectOfInterest;
//                    CGRect xr = [self.capturePreviewLayer rectForMetadataOutputRectOfInterest:CGRectMake(0, 0, 1, 1)];
//                    NSLog(@"");
//                }
            }
